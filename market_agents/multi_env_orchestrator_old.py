...
from market_agents.logger_utils import (
    print_ascii_art, log_section, log_agent_init, log_perception, log_action, log_reflection,
...
class GroupChatConfig(BaseModel):
    ...
    group_size: int = Field(default=5)
...
class Orchestrator:
    ...
    def process_group_chat_actions(self, actions_map: Dict[str, LLMOutput]) -> GroupChatGlobalAction:
        ...
        for agent_id, action_output in actions_map.items():
            try:
                ...
                log_action(logger, agent.index if agent else "Unknown", f"Message: {group_chat_message.content}")
    ...
    async def generate_topic_for_batch(self, batch_index: int, topic_proposer: MarketAgent, round_num: int) -> str:
        group_chat_env_name = f"group_chat_batch_{batch_index}"
        ...
        if auction_env and round_num > 1:
            ...
            if not latest_observation:
                logger.warning("No global state available for auction; reverting to a general topic prompt.")
        ...
        logger.debug(f"Topic proposer {topic_proposer.id} for batch {batch_index} generated action: {topic_action}")
        ...
        try:
            ...
            new_topic = f"Default topic for batch {batch_index}: General {good_name} market discussion"

        # Colored print for the new topic
        print(f"{Fore.CYAN}Generated new topic for batch {batch_index}:{Fore.YELLOW} {new_topic}{Style.RESET_ALL}")
        logger.info(f"Generated new topic for batch {batch_index}: {new_topic}")
        return new_topic
        ...
        try:
            for round_num in range(1, self.config.max_rounds + 1):
                ...
                for batch_index, batch in enumerate(self.agent_batches):
                    ...
                    # Select a random topic proposer from the batch
                    topic_proposer = random.choice(batch)
                    
                    # Generate a topic for the batch
                    new_topic = await self.generate_topic_for_batch(batch_index, topic_proposer, round_num)
                    
                    # Update the environment with the new topic
                    self.current_topic = new_topic
                    self.environments[group_chat_env_name].mechanism.current_topic = new_topic
                    logger.info(f"Updated topic for {group_chat_env_name}: {new_topic}")
