...
class SimulationDataInserter:
    ...
    def insert_round_data(self, round_num, agents, agent_dict, trackers, config):
        ...
        try:
            ...
            logging.exception("Exception details:")
            raise
    def insert_article_summaries(self, article_data: List[Dict[str, Any]]):
        """
        Insert article summaries generated by the research agent into the database.
        
        Args:
            article_data (List[Dict[str, Any]]): List of article summary dictionaries containing:
                - url: str
                - title: str
                - content: str
                - timestamp: datetime
        ...
        query = """
            INSERT INTO article_summaries (
                id, url, title, content, timestamp, status, 
                basic_summary, key_points, market_impact, trading_implications,
                agent_id, extraction_method
            )
            ...
            ON CONFLICT (url) DO UPDATE SET
                ...
                basic_summary = EXCLUDED.basic_summary,
                key_points = EXCLUDED.key_points,
                market_impact = EXCLUDED.market_impact,
                trading_implications = EXCLUDED.trading_implications,
            ...
            with self.conn.cursor() as cur:
                for article in article_data:
                    ...
                    cur.execute(query, (
                        ...
                        summary.get('market_impact', ''),
                        summary.get('trading_implications', ''),
                        article['agent_id'],
                        article['extraction_method']
                    ))
                
                self.conn.commit()
                logging.info(f"Successfully inserted {len(article_data)} article summaries")
                
        except Exception as e:
            self.conn.rollback()
            logging.error(f"Error inserting article summaries: {str(e)}")
            raise
