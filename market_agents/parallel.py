...
class Orchestrator:
    ...
    def insert_round_data(self, round_num):
        ...
        try:
            ...
            memories_data = [
                {
                    ...
                    'step_id': round_num,
                    'memory_data': agent.memory[-1] if agent.memory else {}
                }
                for agent in self.agents
            ]
            ...
            #     if agent.economic_agent.is_buyer:
            #         if good_name not in agent.economic_agent.value_schedules:
            #             logging.warning(f"Good name '{good_name}' not found in value_schedules for agent {agent.id}. Using first available schedule.")
            #             good_name = next(iter(agent.economic_agent.value_schedules))
            #         schedule = agent.economic_agent.value_schedules[good_name]
            #     else:
            #         if good_name not in agent.economic_agent.cost_schedules:
            #             logging.warning(f"Good name '{good_name}' not found in cost_schedules for agent {agent.id}. Using first available schedule.")
            #             good_name = next(iter(agent.economic_agent.cost_schedules))
            #         schedule = agent.economic_agent.cost_schedules[good_name]

            #     schedules_data.append({
            #         'agent_id': str(agent.id),
            #         'is_buyer': agent.economic_agent.is_buyer,
            #         'values': schedule.values,
            #         'initial_endowment': {
            #             'cash': agent.economic_agent.endowment.initial_basket.cash,
            #             'goods': agent.economic_agent.endowment.initial_basket.goods_dict
            #         }
            #     })
            ...
            # else:
            #     logging.warning("No valid schedules data to insert.")
        ...
        # Allocations data
            ...
            orders_data = [
                {
                    ...
                    'price': order.price,
                    'base_value': order.base_value if hasattr(order, 'base_value') else None,
                    'base_cost': order.base_cost if hasattr(order, 'base_cost') else None
                }
                for agent in self.agents
            ...
            for agent in self.agents:
                if hasattr(agent, 'last_observation') and agent.last_observation:
                    reflections_data.append({
                        ...
                        'environment_info': agent.last_perception.get('environment_info', {}) if hasattr(agent, 'last_perception') else {},
                        'last_action': agent.last_action if hasattr(agent, 'last_action') else None,
                        'reward': agent.last_observation.get('reward', 0),
                        'previous_strategy': agent.last_perception.get('strategy', '') if hasattr(agent, 'last_perception') else ''
                    })
            ...
            for env_name, tracker in self.trackers.items():
                for trade in tracker.all_trades:
                    buyer = next(agent for agent in self.agents if str(agent.id) == str(trade.buyer_id))
                    seller = next(agent for agent in self.agents if str(agent.id) == str(trade.seller_id))
        ...
        for agent in self.agents:
            ...
            if agent_observation:
                ...
                agent.last_perception = {
                    'monologue': agent_observation.get('monologue', ''),
                    'strategy': agent_observation.get('strategy', ''),
